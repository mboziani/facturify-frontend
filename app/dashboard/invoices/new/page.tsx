...
export default function NewInvoicePage() {
    ...
    const onSubmit = async (data: InvoiceFormData, sendImmediately = false) => {
        if (!currentCompany) return;
        ...
        try {
            ...
            if (sendImmediately) {
                await invoiceApi.markAsSent(invoice.id);
            }
            ...
            <form onSubmit={handleSubmit((data) => onSubmit(data, false))} className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-slate-200">
                    ...
                    <button
                        type="submit"
                        disabled={isSaving}
                        className="px-6 py-3 bg-slate-600 text-white rounded-lg font-semibold hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
